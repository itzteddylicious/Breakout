...
function PlayState:init()
    ...
    self.balls = {}
    -- timer to control when to spawn a KeyPowerUp
    self.timer = 0
    -- flag to control if the player has picked the key PowerUp
    self.powerUpKeyFlag = false
    -- flag to control whether the keyBlock is in play
...
function PlayState:enter(params)
    ...
    -- we use this to control if we are in a level where there is a Locked brick
...
function PlayState:update(dt)
    ...
    for unused, powerUp in pairs(self.powerUps) do
        ...
        if powerUp:collides(self.paddle) then
            ...
            if powerUp.tier == PU_MULT_BALL then
                -- add two new balls based on the main ball
                -- tweak the angle in different direction of every new ball
    ...
    for k, ball in pairs(self.balls) do
        ...
        for k, brick in pairs(self.bricks) do
            ...
            if brick.inPlay and ball:collides(brick) then
                ...
                if (not self.powerUpBallLock and table.getn(self.balls) == 1 and self.numBricksHit >= 2) then
                    ...
                    -- lock power up until the ball hits paddle again
        ...
        if ball.y >= VIRTUAL_HEIGHT then
            ...
            if table.getn(self.balls) == 1 then
                -- if its the last ball and the smallest paddle size
    ...
    if self.timer >= self.keySpawnTimer and not self.powerUpFlag and not self.lockedBrickRemoved then
